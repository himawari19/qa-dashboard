...
function generateCoverageAlerts(
  byProject: Array<{ project: string; total: number; passed: number; failed: number; blocked: number; pending: number; coverageRate: number; passRate: number }>,
  trendRows: TrendRow[],
): CoverageAlert[] {
  const alerts: CoverageAlert[] = [];
  const CRITICAL_THRESHOLD = 50;
  const WARNING_THRESHOLD = 70;
  const DROP_THRESHOLD = 10; // Alert if pass rate drops by 10%+ compared to recent average

  // Check each project for low coverage
  for (const proj of byProject) {
    ...
    // High pending rate (not executed)
  ...
  // Check trend for drops
  const reversedTrend = [...trendRows].reverse(); // Most recent first
...
export async function GET() {
  ...
  try {
    ...
    const stats = statsRows[0] || { total: 0, passed: 0, failed: 0, blocked: 0, pending: 0 };

    // Coverage trend from execution runs (last 12 runs)
    ...
    // Coverage by project (aggregated)
    ...
    return NextResponse.json({
      stats: {
        ...
        coverageRate: Number(stats.total) > 0 ? Math.round(((Number(stats.passed) + Number(stats.failed)) / Number(stats.total)) * 100) : 0,
        passRate: Number(stats.total) > 0 ? Math.round((Number(stats.passed) / Number(stats.total)) * 100) : 0,
      },
      byProject,
      bySuite,
    ...
    console.error("Test coverage API error:", error);
