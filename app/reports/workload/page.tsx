...
export default function WorkloadPage() {
  ...
  const getScoreColor = (level: string) => {
    switch (level) {
      ...
      case 'high':     return 'text-orange-600 bg-orange-50 border-orange-100 dark:bg-orange-950/20 dark:border-orange-900/30';
      case 'medium':   return 'text-amber-600 bg-amber-50 border-amber-100 dark:bg-amber-950/20 dark:border-amber-900/30';
      default:         return 'text-sky-600 bg-sky-50 border-sky-100 dark:bg-sky-950/20 dark:border-sky-900/30';
    }
  };
      ...
      actions={
        <div className="relative">
          ...
          <input
            ...
            className="h-10 w-64 rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-xs font-semibold shadow-sm outline-none transition focus:ring-2 focus:ring-blue-500/20 dark:border-white/10 dark:bg-slate-900"
          />
        </div>
      }
    >
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-black text-slate-900 dark:text-white">{efficiency}%</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-white/10 dark:bg-slate-900">
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h3 className="text-lg font-black text-slate-900 dark:text-white">Workload Heatmap</h3>
          ...
          {loading ? (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="h-32 animate-pulse rounded-2xl bg-slate-50 dark:bg-white/5" />
              ))}
            </div>
          ) : filtered.length === 0 ? (
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="mb-4 flex items-center justify-between">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-900 text-[10px] font-black text-white dark:bg-white dark:text-slate-900">
                    ...
                    <p className="mb-4 truncate text-[10px] font-bold uppercase tracking-widest text-slate-400">{getRoleLabel(item.role)}</p>

                    <div className="grid grid-cols-2 gap-2">
                      <div className="rounded-xl border border-white/50 bg-white/50 p-2.5 dark:border-white/5 dark:bg-slate-900/50">
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Tasks</p>
                        <p className="text-sm font-black text-slate-900 dark:text-white">{item.tasks}</p>
                      </div>
                      <div className="rounded-xl border border-white/50 bg-white/50 p-2.5 dark:border-white/5 dark:bg-slate-900/50">
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Plans</p>
                        <p className="text-sm font-black text-slate-900 dark:text-white">{item.plans}</p>
                      </div>
                    </div>

                    <div className="mt-4 flex items-center justify-between">
                      <span className="text-[10px] font-bold text-slate-400">Workload Score</span>
                      <span className={cn(
                        "rounded-full px-2 py-0.5 text-[10px] font-black",
                        item.level === 'critical' ? 'bg-rose-500 text-white' : 'bg-slate-900 text-white dark:bg-white dark:text-slate-900'
                      )}>
                        {item.score}
                      </span>
                    </div>
        ...
        {!loading && filtered.length > 0 && (
          <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm dark:border-white/10 dark:bg-slate-900">
            <div className="border-b border-slate-100 bg-slate-50/50 px-6 py-4 dark:border-white/5 dark:bg-white/5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  {filtered.map(item => (
                    <tr key={item.id} onClick={() => openDetail(item)} className="cursor-pointer transition-colors hover:bg-slate-50/50 dark:hover:bg-white/5">
                      ...
                      <td className="px-6 py-4 font-medium text-slate-600 dark:text-slate-300">{getRoleLabel(item.role)}</td>
          ...
          <div className="relative flex max-h-[85vh] w-full max-w-xl flex-col rounded-2xl bg-white shadow-2xl animate-in slide-in-from-bottom-4 duration-300 sm:slide-in-from-bottom-0 dark:bg-slate-900">
            <div className="flex items-center justify-between border-b border-slate-200/60 px-4 py-3 dark:border-white/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-blue-500">Resource Detail</p>
                  <h2 className="text-sm font-black text-slate-900 dark:text-white">{detail.member.name}</h2>
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">{getRoleLabel(detail.member.role)}</p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-3 gap-3 border-b border-slate-200/60 px-4 py-3 dark:border-white/10">
              <div className="rounded-xl bg-blue-50 px-3 py-2 dark:bg-blue-950/30">
                ...
                <p className="text-lg font-black text-slate-900 dark:text-white">{detail.member.tasks}</p>
              </div>
              <div className="rounded-xl bg-blue-50 px-3 py-2 dark:bg-blue-950/30">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Plans</p>
                <p className="text-lg font-black text-slate-900 dark:text-white">{detail.member.plans}</p>
              </div>
              <div className="rounded-xl bg-blue-50 px-3 py-2 dark:bg-blue-950/30">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Score</p>
                <p className="text-lg font-black text-slate-900 dark:text-white">{detail.member.score}</p>
              </div>
            </div>

            <div className="flex-1 overflow-y-auto px-4 py-3">
              <p className="mb-3 text-[9px] font-black uppercase tracking-widest text-slate-400">Active Items</p>
              {detailLoading ? (
                <div className="space-y-2">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="h-14 animate-pulse rounded-xl bg-slate-50 dark:bg-white/5" />
                  ))}
                </div>
                ...
                <div className="space-y-2">
                  {detail.items.map((item, i) => (
                    <Link
                      ...
                      prefetch={false}
                      onClick={() => setDetail(null)}
                      className="group flex items-center gap-3 rounded-xl border border-slate-100 p-3 transition hover:border-blue-200 hover:bg-blue-50/40 dark:border-white/5 dark:hover:border-blue-800/40 dark:hover:bg-blue-950/20"
                    >
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-xs font-bold text-slate-800 group-hover:text-blue-700 dark:text-slate-200 dark:group-hover:text-blue-400">
                          {item.label}
                        </p>
                        <p className="mt-0.5 text-[10px] font-semibold text-slate-400">{item.sub}</p>
                      </div>
            ...
            <div className="flex items-center justify-end border-t border-slate-200/60 px-4 py-3 dark:border-white/10">
              <button
                onClick={() => setDetail(null)}
