...
export default function SqlPlayground() {
  ...
  return (
    <PageShell
      eyebrow="SQL Playground"
      title="SQL Playground"
      description="Execute direct database queries for data verification."
      actions={
        <button
          onClick={runQuery}
          disabled={loading}
          ...
          <div className="overflow-x-auto max-h-[500px]">
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-white z-10 border-b border-slate-200">
                <tr>
                  {columns.map((col) => (
                    <th key={col} className="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {result.length === 0 && !loading && (
                  <tr>
                    <td colSpan={100} className="px-6 py-12 text-center text-slate-400 text-sm italic">
