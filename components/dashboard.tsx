...
function Drawer({ title, subtitle, items, loading, onClose, viewAllHref }: {
 ...
 <div ref={ref} className="h-full w-full max-w-sm bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
 {/* Header */}
 ...
 <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-md text-slate-400 hover:bg-slate-100 transition">
 ...
 className="flex items-center gap-3 rounded-md border border-slate-100 p-3 hover:border-blue-200 hover:bg-blue-50/40 transition group">
 <div className="flex-1 min-w-0">
 <p className="text-sm font-semibold text-slate-800 truncate group-hover:text-blue-700">{item.label}</p>
 ...
 className="flex items-center justify-center gap-2 h-10 rounded-md bg-slate-900 text-white text-sm font-bold hover:bg-blue-600 transition">
...
"flex flex-col gap-3 p-5 text-left transition-all hover:-translate-y-1 hover:shadow-xl group glass-card bg-white rounded-2xl",
 ...
 {caption && (
 <p className="text-[11px] font-bold text-slate-400 mt-1 truncate">{caption}</p>
 )}
 </div>
 </button>
 );
...
const ChartTip = ({ active, payload, label }: any) => {
 ...
 <span className="font-semibold text-slate-700">{p.name}: {p.value}</span>
 </div>
 ))}
 </div>
 );
...
"Sessions": { href:"/test-sessions" },
 ...
 const statCards = [
 { label:"Open Tasks", icon: <Kanban size={20} weight="bold" className="text-blue-600" />, color:"bg-blue-50" },
 { label:"Bug Entries", icon: <Bug size={20} weight="bold" className="text-rose-500" />, color:"bg-rose-50" },
 { label:"Test Cases", icon: <Checks size={20} weight="bold" className="text-emerald-500" />, color:"bg-emerald-50" },
 { label:"Test Suites", icon: <Table size={20} weight="bold" className="text-indigo-500" />, color:"bg-indigo-50" },
 { label:"Sessions", icon: <PlayCircle size={20} weight="bold" className="text-amber-500" />, color:"bg-amber-50" },
 ];
 ...
 className="inline-flex h-9 items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 text-xs font-bold text-slate-600 hover:bg-slate-50 transition">
 <Printer size={15} weight="bold" /> Print
 </button>
 <div className="h-9 w-px bg-slate-200 self-center" />
 ...
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Bugs by Module</h3>
 ...
 <div className="flex items-center justify-between mb-1">
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Bug Severity</h3>
 ...
 <div className="flex items-center justify-between mb-1">
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Task Status</h3>
 ...
 <div className="flex items-center justify-between mb-1">
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Execution Trend</h3>
 ...
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Active Sprint</h3>
 <CalendarBlank size={15} className="text-slate-400" weight="bold" />
 </div>
 {sprintInfo ? (
 <>
 <div className="flex-1">
 <p className="text-lg font-black text-slate-900 leading-tight">{sprintInfo.name}</p>
 ...
 <div className="flex items-center justify-between mb-1">
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Sprint Burndown</h3>
 ...
 <div className="min-w-0 rounded-2xl glass-card bg-white p-5">
 <div className="flex items-center justify-between mb-1">
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Pass Rate per Sprint</h3>
 ...
 <div className="min-w-0 rounded-2xl glass-card bg-white p-5">
 <div className="flex items-center justify-between mb-1">
 <h3 className="text-xs font-black uppercase tracking-widest text-slate-700">Bug Trend</h3>
 ...
 {/* Quality shield */}
 <div className="rounded-xl bg-slate-900 p-5 text-white relative overflow-hidden">
 ...
 className="flex w-full items-center gap-2 rounded-md bg-slate-50 p-2.5 text-left hover:bg-rose-50 transition group">
 <XCircle size={14} weight="fill" className="text-rose-500 shrink-0" />
 <span className="flex-1 text-xs font-semibold text-slate-700 truncate group-hover:text-rose-700">{bug.title}</span>
 ...
 className="flex w-full items-center gap-2 rounded-md bg-slate-50 p-2.5 text-left hover:bg-blue-50 transition group">
 <CheckCircle size={14} weight="fill" className="text-blue-500 shrink-0" />
 <span className="flex-1 text-xs font-semibold text-slate-700 truncate group-hover:text-blue-700">{task.title}</span>
 ...
 <div className="absolute left-[22px] top-0 bottom-0 w-px bg-gradient-to-b from-blue-200 via-slate-200 to-transparent" />
 {activity.map((item, i) => {
 const actionColor = item.action ==="create" ?"border-emerald-300 bg-emerald-50"
 : item.action ==="delete" ?"border-rose-300 bg-rose-50"
 ...
 <p className="text-xs font-semibold text-slate-800 leading-snug line-clamp-2">{item.summary}</p>
 <div className="flex items-center gap-1.5 mt-1">
 <span className="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-500">{formatDisplayText(item.entityType)}</span>
 <span className="text-[11px] text-slate-400">{item.createdAt?.slice(0, 16).replace("T","")}</span>
 </div>
 </div>
 </div>
 );
 ...
 <div>
 <h3 className="text-xs font-black uppercase tracking-widest text-amber-800">Bottleneck Detector</h3>
 <p className="text-xs text-amber-600">{bottlenecks.length} item{bottlenecks.length !== 1 ?"s" :""} stuck in a status too long</p>
 </div>
 </div>
 <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
 {bottlenecks.slice(0, 9).map(b => (
 <Link key={b.id} href={b.href} prefetch={false} className="flex items-start gap-2.5 rounded-md border border-amber-200 bg-white px-3 py-2.5 hover:border-amber-400 transition group">
 <div className="shrink-0 mt-0.5">
 <span className={cn(
"inline-flex h-5 items-center rounded px-1.5 text-[10px] font-black uppercase tracking-wider",
 b.module ==="Bug" ?"bg-red-100 text-red-600" :"bg-blue-100 text-blue-600"
 )}>{b.module}</span>
 </div>
 <div className="flex-1 min-w-0">
 <p className="text-xs font-bold text-slate-800 truncate group-hover:text-amber-700 transition">{b.title}</p>
...
function EmptyChart({ label }: { label: string }) {
 ...
 <ChartBar size={18} weight="bold" className="text-slate-300" />
 </div>
 <p className="text-xs text-slate-400 italic max-w-[200px]">{label}</p>
 </div>
 );
}
