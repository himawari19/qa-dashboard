...
export function ModuleWorkspaceCell({
 ...
 key={column.key}
 rowSpan={rowSpan}
 className="max-w-64 border-b border-slate-200/60 px-4 py-3 text-sm text-slate-700 align-top whitespace-pre-wrap break-words"
 >
 ...
 <td
 key={column.key}
 className={cn("max-w-64 border-b border-slate-200/60 px-4 py-3 text-sm text-slate-700 align-top whitespace-pre-wrap break-words")}
 >
 {column.internalLink && value ? (
 <Link href={column.internalLink(row)} className="break-all text-blue-700 font-semibold hover:underline">
 <HighlightText text={String(value)} query="" linkify={false} />
 ...
 className="rounded-sm bg-sky-50 px-2 py-1 text-xs font-semibold text-sky-700 hover:bg-sky-100"
 >
 ...
 <div
 className={cn(
"max-w-[280px] rounded-sm bg-slate-50 p-2 text-xs leading-relaxed break-words cursor-default whitespace-normal",
 ...
 <div
 className={cn(
"max-w-[240px] rounded-sm bg-slate-50 p-2 text-xs leading-relaxed break-words whitespace-pre-wrap cursor-default",
 module ==="test-suites" ?"min-h-12" :"h-24 overflow-y-auto",
 )}
 title={String(value ||"")}
 >
